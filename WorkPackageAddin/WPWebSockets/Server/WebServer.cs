...
{
    /// <summary>
    /// The server that is listening for connections.
    /// </summary>
    public class WebServer : IDisposable
    {
        ...
        public List<IDisposable> GetConnections()
        {
        ...
        private static ConnectionDetails GetConnectionDetails(Stream stream, TcpClient tcpClient)
        {
            ...
            if (getRegexMatch.Success)
            {
                ...
                {
                    return new ConnectionDetails(stream, tcpClient, path, ConnectionType.WebSocket, header,0);
                }
                else
                {
                    return new ConnectionDetails(stream, tcpClient, path, ConnectionType.Http, header,0);
                }
            }
            else
            {
                return new ConnectionDetails(stream, tcpClient, string.Empty, ConnectionType.Unknown, header,0); 
            }
        }
        ...
        private Stream GetStream(TcpClient tcpClient)
        {
            ...
            catch (AuthenticationException e)
            {
                ...
                _logger.Warning(this.GetType(), "{0} exception thrown", e.Message);
                HttpHelper.WriteHttpHeader("HTTP/1.1 401 Unauthorized", stream);
                throw;
            }
        }
        ...
        private void HandleAsyncConnection(IAsyncResult res)
        {
            try
            {
                ...
                using (TcpClient tcpClient = _listener.EndAcceptTcpClient(res))
                {
                    ...
                    using (IService service = _serviceFactory.CreateInstance(connectionDetails))//connectionDetails
                    {
                        try
                        {
